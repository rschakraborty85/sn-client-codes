...
WSDSearchServiceSNC.prototype = {
	...
	search: function(searchRequest, reservableModule, alternateSearchOptions) {
		...
		return this.searchForReservableUnits(
			...
			alternateSearchOptions.reservable_filter ? alternateSearchOptions.reservable_filter : reservableModule.reservable_filter,
	...
	searchForReservableUnits: function(
		...
		var reservableSearchOutput = { hasMore: false, reservableUnits: [], reservableContainers: [], nextItemIndex: -1 };
		var hasReservedReservables = !WSDUtils.nullOrEmpty(reservedReservableIds);
		var hasNextItemIndex = typeof nextItemIndex === 'number' && !isNaN(nextItemIndex);
		var searchLimit = this.getSearchLimit();

		reservableColumns = WSDUtils.arrayHasElement(reservableColumns) ? reservableColumns : ['sys_id', 'name'];
		...
		if (!hasNextItemIndex && hasReservedReservables) {
			var resolvedReservedItemsResult = this._fetchReservedUnitsOnSearch(startGdt,
				endGdt,
				requireApproval,
				reservableTable,
				reservableFilter,
			...
			reservableSearchOutput.reservableUnits = resolvedReservedItemsResult.reservableUnits;
			reservableSearchOutput.reservableContainers = resolvedReservedItemsResult.reservableContainers;
		}

		/** ACTUAL SEARCH */
		var reservableEQArr = [];
		var parsedExtraConditions = this._resolveExtraConditionEncodedQuery(extraConditions);
		...
		var reservableEQ = reservableEQArr.join("^")
			.replace(/\^EQ/, '');
		...
		var windowStart = 0;
		if (hasNextItemIndex && nextItemIndex >= 0)
		...
		this._addApplicableReservableTableQuery(reservableGr, parsedExtraConditions);

		if (sortBy) {
			var sortByField = selectionType === WSDConstants.RESERVABLE_MODULE_SELECTION_TYPE.container ? reservableContainerField : this.DEFAULT_SORT_FIELD;
		...
		if (this._isSpaceMgmtApplicable(reservableType))
			this._addSpaceMgmtQueries(reservableGr, parsedExtraConditions, requireCostCenterDepartmentCheck);


		if (selectionType === WSDConstants.RESERVABLE_MODULE_SELECTION_TYPE.container && reservableContainerField)
			reservableGr.addNotNullQuery(reservableContainerField)

		reservableGr.query();

		WSDLogger.debug('WSDSearchServiceSNC.searchForReservableUnits', 'Constructed (primary) query on search',
			{ table: reservableTable, encodedQuery: reservableGr.getEncodedQuery()});

		// adding one to check if there is more to request to load
		var pageSizePlusOne = (pageSize - reservableSearchOutput.reservableUnits.length) + 1;
		...
		var allPossibleMatchedItems = reservableSearchOutput.reservableUnits.concat(resolvedItemsResult.reservableUnits);

		// resolve and construct the applicable filter data that matches reservables result
		reservableSearchOutput.filter = this._resolveAndConstructFilterData(reservableType, allPossibleMatchedItems);
		reservableSearchOutput.totalProcessed = resolvedItemsResult.totalProcessed;

		reservableSearchOutput.reservableUnits = allPossibleMatchedItems.filter(function(item) {
			return item.includedInResult || item.is_reserved;
		});

		// resolve possible pagination data
		reservableSearchOutput.hasMore = reservableSearchOutput.reservableUnits.length >= pageSizePlusOne;
		if (reservableSearchOutput.hasMore) {
			...
			reservableSearchOutput.nextItemIndex = supposedStartIndex + resolvedItemsResult.recordProcessedForResult;
		}
	...
	_fetchReservedUnitsOnSearch: function(
		...
		return this._resolveReservablesByGr(
			...
			function(reservableGr, reservableUnit) {
				reservableUnit.is_available = reservableUnit.is_available &&
					(!reservableFilter || GlideFilter.checkRecord(reservableGr, reservableFilter));
	...
	getSearchLimit: function() {
		var searchLimit = WSDUtils.getIntProperty(WSDConstants.SYSTEM_PROPERTY.searchLimit);
	 ...
	 * @private
	 */
	_addApplicableReservableTableQuery: function(reservableGr, parsedExtraConditions) {
		if (!parsedExtraConditions)
			return;
		...
		for (var j = 0; j < parsedExtraConditions.length; j++) {
			...
			if (parsedCondition.isPrimaryCondition)
				reservableGr.addQuery(parsedCondition.key, parsedCondition.operation, parsedCondition.value);
		}
	},
	...
	_resolveExtraConditionEncodedQuery: function(extraConditionsEncodedQuery) {
		if (WSDUtils.nullOrEmpty(extraConditionsEncodedQuery))
			return null;
		...
		for (var i = 0; i < conditionArr.length; i++) {
			...
			if (parsedCondition && !WSDUtils.arrayContainsElement(conditionResult, parsedCondition, 'key'))
	...
	_parseSingleCondition: function(condition) {
		...
		for (var i = 0; i < possibleOperation.length; i++) {
			var arrPair = this._parseSingleConditionToArrayPairUsingKey(condition, possibleOperation[i]);
			if (arrPair) {
				// primary condition implies such condition is applicable on the targeted reservable table.
				var isPrimaryCondition = this.SEARCHABLE_LOCATION_COLUMNS.indexOf(arrPair[0]) >= 0;
	...
	_resolveAndConstructFilterData: function(reservableType, reservableUnits) {
		...
		var filter = isLocation ? {floors: [], standard_services: [], reservable_purposes: []} : {};

		if (!isLocation) {
			// filter for configuration-item type search
			WSDLogger.error('WSDSearchServiceSNC._resolveAndConstructFilterData', 'Filter construction is not yet supported for reservable type: ' + reservableType);
			return filter;
		}
		...
		for (var i = 0; i < reservableUnits.length; i++) {
			...
			// construct floors
			if (!WSDUtils.arrayContainsElement(filter.floors, reservableUnit.floor, 'sys_id'))
				filter.floors.push(reservableUnit.floor);

			// construct standard services
			filter.standard_services = this._resolveFilterData(reservableUnit, filter.standard_services, 'standard_services');

			// construct reservable purposes
			filter.reservable_purposes = this._resolveFilterData(reservableUnit, filter.reservable_purposes, 'reservable_purposes');
		}
	...
	_resolveReservablesByGr: function(
		...
		var hasNonPrimaryConditions = WSDUtils.arrayHasElement(nonPrimaryConditions);
		var isSelectionTypeContainer = selectionType === WSDConstants.RESERVABLE_MODULE_SELECTION_TYPE.container;

		// process each reservable unit, check availability and check against extra condition
		while (reservableGr.next()) {
			totalProcessed++;
			if (!resultCompleted)
				recordProcessedForResult++;

			// when there is a non primary extra condition: prepare the reservable unit output first
			var reservableUnit = null;
			if (hasNonPrimaryConditions && (includeStandardServices || includeReservablePurposes)) {
				reservableUnit = this.getReservableOutput(reservableGr, reservableType, reservableColumns, requireApproval, includeStandardServices, includeReservablePurposes);

				// check if reservable unit satisfy the nonprimary conditions
				if (!this._isReservableMatchedExtraConditions(reservableUnit, nonPrimaryConditions))
					continue;
			}

			if (isSelectionTypeContainer && WSDUtils.nullOrEmpty(reservableGr[reservableContainerField]))
				continue;

			var reservableQuantity = reservableQuantityField && !isNaN(reservableGr.getValue(reservableQuantityField)) ? parseInt(reservableGr.getValue(reservableQuantityField)) : 1;
			...
			var includeReservable = includeUnavailableUnits || availability.is_available;
			if (includeReservable) {
				reservableUnit = reservableUnit
					? reservableUnit
					: this.getReservableOutput(reservableGr, reservableType, reservableColumns, requireApproval, includeStandardServices, includeReservablePurposes);
				reservableUnit.is_available = availability.is_available;
				reservableUnit.reservations = availability.reservations;

				if (callback)
					callback(reservableGr, reservableUnit);

				if (isSelectionTypeContainer) {
					// CONTAINER selection type: process container data (only available must be included)
					reservableUnit.container = this._getContainerDetails(reservableType, reservableGr, reservableContainerField); // reservableUnit[reservableContainerField];
					if (reservableUnit.container && !WSDUtils.arrayContainsElement(reservableContainers, reservableUnit.container, 'sys_id') && reservableUnit.is_available) {
	 ...
	 */
	_getContainerDetails: function(reservableType, reservableGr, reservableContainerField) {
		if (reservableType === WSDConstants.RESERVABLE_TYPE.location) {
			var refRecord = !WSDUtils.nullOrEmpty(reservableGr[reservableContainerField]) && reservableGr[reservableContainerField].getRefRecord();
		...
		// CI
		return this.recordUtils.getReferenceObject(reservableGr, reservableContainerField);
	},
	 ...
	 * @private
	 */
	_isReservableMatchedExtraConditions: function(reservableUnit, nonPrimaryConditions) {
		if (!WSDUtils.arrayHasElement(nonPrimaryConditions))
			return true;
			...
			switch (condition.key) {
				...
				case 'reservable_purposes':
					if (!WSDUtils.arrayHasElement(reservableUnit[condition.key]))
						return false;
					if (!this._ArrMatchedConditionValue(arrToVerify, condition.value, condition.operation))
						return false;
					break;
				default:
					break;
	...
	_ArrMatchedConditionValue: function(arrToVerify, conditionValue, operation) {
		...
		for (var j = 0; j < values.length; j++) {
			var contain = WSDUtils.arrayContainsElement(arrToVerify, { sys_id: values[j] }, 'sys_id')

			if (operation === '=' && !contain)
				return false;
			else if (operation === 'IN' && contain)
				return true;
		}
	 ...
	 */
	getReservableOutput: function(reservableGr, reservableType, reservableColumns, requireApproval, includeStandardServices, includeReservablePurposes) {
		...
		if (reservableColumns.indexOf('name') < 0)
			reservableColumns.push('name');

		var reservableUnit = this.recordUtils.getObjectFromGlideRecord(reservableGr, reservableColumns);
		reservableUnit.require_approval = WSDUtils.safeBool(requireApproval);
		reservableUnit.location_type = this.recordUtils.getReferenceObject(reservableGr, 'location_type');

		// handle output for location type
		if (reservableType === WSDConstants.RESERVABLE_TYPE.location) {
			reservableUnit.image = this.recordUtils.getImageFromField(reservableGr, 'image');

			// fetch standard services for location
			if (includeStandardServices)
				reservableUnit.standard_services = this.standardServiceHelper.getLocStandardServiceById(reservableGr.getValue('sys_id'));

			// fetch reservable purposes for location
			if (includeReservablePurposes)
				reservableUnit.reservable_purposes = this.reservablePurposeService.getLocReservablePurposeById(reservableGr.getValue('sys_id'));
		}
	...
	saveSearch: function(reservableModule, shift, searchQuery, sortBy) {
		...
		gs.getUser().savePreference(WSDConstants.USER_PREFERENCE.lastSearchRequest, searchObjStr);
	},
	...
	getInitSearchConfig: function(preSelectedReservableModule) {
		var searchObj;
		var searchObjStr = gs.getUser().getPreference(WSDConstants.USER_PREFERENCE.lastSearchRequest);
		if (!searchObjStr || searchObjStr.length === 0)
			searchObj = this.getFirstSearchConfig();
		else
			searchObj = JSON.parse(searchObjStr);
	...
	_resolveSearchConfigData: function(searchObj) {
		...
		if (searchObj.building)
			searchObjWithDisplay.building = this.getBuildingFromId(searchObj.building);

		if (searchObj.floors)
			searchObjWithDisplay.floors = this.getFloorsFromIds(searchObj.floors);

		if (searchObj.reservable_module)
			searchObjWithDisplay.reservable_module = this.reservableModuleService.getReservableModuleAsChoiceById(searchObj.reservable_module);

		if (searchObj.shift) {
			// fetch shift data
			searchObjWithDisplay.shift = this.shiftService.validateShiftIsAvailableTodayAndGetDetails(searchObj.shift);
		}
	...
	getFirstSearchConfig: function() {
		...
		if (String(userGr.location.sys_class_name) === WSDConstants.TABLES.Building.name)
			return {
	...
	_addSortQuery: function(sortBy, gr, fieldNameToSort) {
		fieldNameToSort = fieldNameToSort ? fieldNameToSort : this.DEFAULT_SORT_FIELD;
		if (sortBy === WSDConstants.SEARCH_SORT_BY.z_a)
			gr.orderByDesc(fieldNameToSort);
		else
			gr.orderBy(fieldNameToSort);
	},
	...
	*/
	_isSpaceMgmtApplicable: function(reservableType) {
		return GlidePluginManager.isActive('com.sn_wsd_space_mgmt') && reservableType === WSDConstants.RESERVABLE_TYPE.location;
	},
	...
	* @return {String}
	*/
	_addSpaceMgmtQueries: function(reservableGr, parsedExtraConditions, requireCostCenterDepartmentCheck) {
		//Assignment Type query
		reservableGr.addNullQuery('wsd_assignment_type').addOrCondition('wsd_assignment_type', sn_wsd_core.WPConstants.ASSIGNMENT_TYPE.FLEX);
		...
		if(requireCostCenterDepartmentCheck) {
			...
			var ccDeptQuery = new sn_wsd_spcmgmt.WPUserProfile().getCCDeptQuery(userId, buildingId);

			if(!gs.nil(ccDeptQuery))
				reservableGr.addEncodedQuery(ccDeptQuery);
		}		
	},
