...
WSDAvailabilityService.prototype = Object.extendsObject(
  ...
  {
    /** @member {sn_wsd_core.WPBlockLocationUtils} */
    blcLocationUtils: new sn_wsd_core.WPBlockLocationUtils(),
      ...
      try {
        if (reservableType === WSDConstants.RESERVABLE_TYPE.location) {
          ...
          if (blcLoc.isBlocked)
            return {
              is_available: false,
              reservations: [this._getBlockedLocationDetails(blcLoc.record)],
            };
        }
      } catch (error) {
        gs.error(
          "RC checkReservableAvailability function " +
            reservableSysId +
            "\t" +
            error
        );
      ...
      while (reservationGr.next()) {
        ...
        if (
          ...
          if (includeReservationsWithinDays)
            reservations.push(
              this._getReservationObjectFromGlideRecord(reservationGr, false)
            );
        ...
        if (isReservablePlaceholderType) {
          ...
          if (isCollided)
            quantifiedReservations.push(
              this._getReservationMatchingData(reservationGr, currentReservable)
            );

          var rsvBlock = this._flattenReservationBlock(quantifiedReservations);
