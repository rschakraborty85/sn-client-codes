 ...
 function wsdSearchFilter(
  ...
  return {
    ...
    link: function (scope) {
      ...
      var nowMorning = moment(
        now.format(dateFormat) + " " + DEFAULT_DAY_START,
        dateTimeFormat
      );
      ...
      function _fetchFilterInformationFromConfig(initConfig) {
        ...
        if (_.isEmpty(initConfig)) {
          scope.isLoadHandled = true;
          return;
        }
        var selectSeat = "";
      ...
      function moduleChange(reservableModule) {
        ...
        wsdReservableSearch.getReservableModule(reservableModuleSysId).then(
          function (reservableModule) {
            reservableModule.reservableMapper = wsdReservableMappingService.createReservableMapper(
              reservableModule
            );
            ...
            if (
              ...
              // @debug RC - STRY2469789
              var onloadMsg = reservableModule.reserve_module_msg;
              // "Reservations can only be made " +
              // reservableModule.max_days_in_future +
              // " days in advance," +
              // " and only one reservation can be active each day";
      ...
      function _createTriggerSearchWatcher() {
        wsdStateService.subscribe("triggerSearch", function (
          old,
          searchOption
        ) {
          var searchValid = evaluateSearchPossibility();
          if (searchValid.isValid) searchForAvailableReservables(searchOption);
      ...
      function searchForAvailableReservables(searchOption) {
        ...
        //console.log('v before calling getAvail');
        //console.log(searchRequestObj);
        //STRY2456995:Workspace Search Changes - Workplace Reservation Management
        searchRequestObj.is_load = !scope.isLoadHandled;
        ...
        return wsdReservableSearch
          ...
          .then(
            function (response) {
              ...
              //console.log('v after calling getAvail');
              //console.log(response);
              //STRY2456995:Workspace Search Changes - Workplace Reservation Management
              if (!scope.isLoadHandled) {
                scope.isLoadHandled = true;
              }
      ...
      function _setSearchReservableFilterState(
        ...
        //console.log('_setSearchReservable')	;
